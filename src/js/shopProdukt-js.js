document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch("https://hifi-corner.herokuapp.com/api/v1/products", {
        ...
        .then(data => {
            ...
            let current = data.find(product => product.sku == paramsId)

            // PRODUKT BILLEDE SECTION
            let billedeSection = document.querySelector(".shop-produkt__billede")
            billedeSection.setAttribute("data-id", current.sku)
            billedeSection.id = 'p' + current.sku
            ...
            // thumbnailSection.setAttribute("data-id", current.sku)
            // thumbnailSection.id = 'p' + current.sku
            ...
            produktOverskrift.setAttribute("data-id", current.sku)
            produktOverskrift.id = 'p' + current.sku
            produktOverskrift.innerHTML = `${current.make}  ${current.model}`;

            let productBrand = document.querySelector(".info__otherProducts_brand")
            productBrand.setAttribute("data-id", current.sku)
            productBrand.id = 'p' + current.sku
            productBrand.innerHTML = `${current.make} `;
            ...
            productlPrice.setAttribute("data-id", current.sku)
            productlPrice.id = 'p' + current.sku
            productlPrice.innerHTML = `${current.price} kr`;

            let productText = document.querySelector(".info__produktTekst")
            productText.setAttribute("data-id", current.sku)
            productText.id = 'p' + current.sku
            productText.innerHTML = `${current.description}`;
